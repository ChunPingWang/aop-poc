...
> **當前分支: `aop4audit`** - 使用 AOP 實作稽核機制

本專案有兩個主要分支，分別展示不同的稽核架構實作方式：

| 分支 | 稽核機制 | 說明 |
|------|----------|------|
| **`aop4audit`** (此分支) | Spring AOP | 使用 `@Auditable` 註解 + `AuditAspect` 攔截 |
| [`main`](https://github.com/ChunPingWang/aop-poc/tree/main) | Domain Events | 使用領域事件 + `@TransactionalEventListener` |

### 兩種方式比較

| 比較項目 | AOP 方式 (此分支) | Domain Events 方式 (main) |
|---------|------------------|--------------------------|
| **實作複雜度** | 較低，快速上手 | 較高，需定義事件類別 |
| **耦合程度** | Service 需標註 `@Auditable` | Service 完全不知道稽核存在 |
| **依賴方向** | Application → Infrastructure | Infrastructure → Application (正確) |
| **資料記錄** | HTTP 層資訊 (endpoint, IP, userAgent) | 業務層資訊 (before/after snapshot) |
| **可測試性** | 需處理 AOP 代理 | 輕鬆 Mock EventPublisher |
| **擴展性** | 修改 Aspect | 新增 EventListener |
| **適用場景** | 快速原型、簡單專案 | 大型專案、嚴格架構 |

### 切換分支

```bash
# 切換到 Domain Events 版本 (main 分支)
git checkout main
